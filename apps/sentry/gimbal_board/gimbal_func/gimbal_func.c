...
// gimbal 电机指针 (DJI/DM 子类, 走基类 API 时显式 (Motor_Base *) 转换)
...
void gimbal_func(Gimbal_Ctrl_Cmd_t *gimbal_cmd, uint16_t *yaw_ecd)
{
    if (gimbal_cmd != NULL)
    {
            ...
            !Module_Offline_get_device_status(small_yaw_motor->base.offline_dev) && !Module_Offline_get_device_status(pitch_motor->base.offline_dev))
        {
            ...
            // 大yaw偏移量，用于实现跟随小yaw
            /* 读小云台原始编码器用于计算偏移 (DJI 子类字段) */
            big_yaw_offset = wt606_device->YawTotalAngle_rad +
                             ((float)(small_yaw_motor->measure.ecd - SMALL_YAW_ALIGN_ECD) * (2.0f * PI / 8192.0f));
    ...
    {
        *yaw_ecd = big_yaw_motor->measure.ecd; /* 原始编码器上报 */
    }
}
