...
static Motor_Base *big_yaw_motor   = NULL; // 大云台yaw电机指针
static Motor_Base *small_yaw_motor = NULL; // 小云台yaw电机指针
...
/*内部函数*/
static void Gimbal_PitchFeedback(Motor_Base *motor, float cur_angle)
{
    /* 重力前馈 */
    float ff_gravity = GRAVITY_K_PITCH * cur_angle + GRAVITY_GAMMA;

    /* 写入控制器的前馈字段，本周期 LQR 输出会自动叠加它 */
    pitch_motor->controller.feedforward_torque = ff_gravity;
}
...
void gimbal_init(void)
{
    ...
    small_yaw_motor = (Motor_Base *)Motor_DJI_Init(&small_yaw_config);
    ...
    Motor_Init_Config_s yaw_config = {
        ...
        .motor_init_info = {.motor_type = GM6020_CURRENT, .gear_ratio = 1, .max_torque = 2.223f, .torque_constant = 0.741f}};
    big_yaw_motor = (Motor_Base *)Motor_DJI_Init(&yaw_config);
...
void gimbal_func(Gimbal_Ctrl_Cmd_t *gimbal_cmd, uint16_t *yaw_ecd)
{
    if (gimbal_cmd != NULL)
    {
        if (!Module_Offline_get_device_status(big_yaw_motor->offline_dev) &&
            !Module_Offline_get_device_status(small_yaw_motor->offline_dev) && !Module_Offline_get_device_status(pitch_motor->offline_dev))
        {
            Gimbal_PitchFeedback(pitch_motor, ins->euler_rad[1]);
            // 大yaw偏移量，用于实现跟随小yaw
            /* 读小云台 DJI 原始编码器用于计算偏移 (仅此处需要子类字段) */
            DJI_Motor_t *small_yaw_dji = MOTOR_GET_DERIVED(small_yaw_motor, DJI_Motor_t);
            big_yaw_offset             = wt606_device->YawTotalAngle_rad +
                                     ((float)(small_yaw_dji->measure.ecd - SMALL_YAW_ALIGN_ECD) * (2.0f * PI / 8192.0f));
            switch (gimbal_cmd->gimbal_mode)
            {
            case gimbal_zero_force:
                Motor_Stop(big_yaw_motor);
                Motor_Stop(small_yaw_motor);
                Motor_Stop(pitch_motor);
                break;
            case gimbal_gyro_mode:
                Motor_Start(big_yaw_motor);
                Motor_Start(small_yaw_motor);
                Motor_Start(pitch_motor);
                // 大小 yaw 协调控制
                Motor_SetRef(big_yaw_motor, big_yaw_offset);
                Motor_SetRef(small_yaw_motor, (gimbal_cmd->yaw * DEGREE_2_RAD) + auto_yaw_offset);
            ...
            case gimbal_auto_mode:
                ...
                if (gimbal_cmd->auto_search == 0)
                {
                    //   大小 yaw 协调控制 - 目标已找到
                    Motor_SetRef(big_yaw_motor, big_yaw_offset);
                    Motor_SetRef(small_yaw_motor, gimbal_cmd->yaw);
                    Motor_SetRef(pitch_motor, gimbal_cmd->pitch);
                    // 目标找到，重置搜索状态，记录当前偏移量
                ...
                else if (gimbal_cmd->auto_search == 1)
                {
                    ...
                    Motor_SetRef(big_yaw_motor, big_yaw_offset);
                    Motor_SetRef(small_yaw_motor, search_yaw_angle);
                    Motor_SetRef(pitch_motor, -search_pitch_ref);
                    // 实时更新偏移量
                ...
                {
                    Motor_SetRef(big_yaw_motor, big_yaw_offset);
                    Motor_SetRef(small_yaw_motor, (float)ins->YawRoundCount * 360.0f * DEGREE_2_RAD);
                    Motor_SetRef(pitch_motor, 0);
                }
                break;
            }
        }
    ...
    // 数据反馈
    if (!Module_Offline_get_device_status(big_yaw_motor->offline_dev) && yaw_ecd != NULL)
    {
        *yaw_ecd = MOTOR_GET_DERIVED(big_yaw_motor, DJI_Motor_t)->measure.ecd; /* 仅上报需要 DJI 原始编码器 */
    }
}
