 ...
 *
 *              本文件 = 协议核心 + 模块入口 合二为一:
 *                - 协议: JustFloat(二进制帧尾) / FireWater(文本)
 *                - 模块入口 Module_VOFA_Init(): 初始化 UART + 创建接收线程
 *
 *              修改(相对 RM2025):
 ...
 *
 *              UART: huart6 (RM2025 原配置), 可通过 VOFA_UART 宏覆盖
 */
#include "vofa.h"
#include "bsp_def.h"
#include "tx_api.h"
...
/* ========== 模块参数 (可在 module_config.cmake 覆盖) ========== */
#ifndef VOFA_UART
...
#endif
#ifndef VOFA_FORMAT
#define VOFA_FORMAT VOFA_FORMAT_JUSTFLOAT
#endif
...
void Module_VOFA_Init(void)
{
    ...
    /* 2. 初始化 VOFA 协议核心 */
    if (VOFA_Init(uart_dev, VOFA_FORMAT, VOFA_FIREWATER_PREFIX) != VOFA_OK)
    {
    ...
    {
        LOG_E("VOFA task create failed (0x%02x)", status);
        return;
    }

    g_initialized = true;
    LOG_I("VOFA module initialized (UART6, format=%d)", VOFA_FORMAT);
}
